...
export function EventLog({ events, players, gameId }: EventLogProps) {
  const queryClient = useQueryClient();

  const undoEventMutation = useMutation({
    mutationFn: async (event: GameEvent) => {
      // TODO: Replace with actual API call (was base44.entities.GameEvent.delete)
      await fetch(`/api/game-events/${event.id}`, { method: 'DELETE' });
    },
    onSuccess: () => {
  ...
  const getEventLabel = (eventType: string): string => {
    const labels: Record<string, string> = {
      shot_2pt_make: '2PT Made',
      shot_2pt_miss: '2PT Missed',
      shot_3pt_make: '3PT Made',
      shot_3pt_miss: '3PT Missed',
      free_throw_make: 'FT Made',
      free_throw_miss: 'FT Missed',
      rebound_off: 'Off Reb',
      rebound_def: 'Def Reb',
      assist: 'Assist',
      steal: 'Steal',
              ...
              return (
                ...
                >
                  <div className="flex-1">
                    ...
                    <Badge
                      className={`${getEventColor(event.eventType)} border-0 text-xs font-semibold`}
                    >
                      {getEventLabel(event.eventType)}
                    </Badge>
                    <div className="text-xs text-slate-500 mt-2">
                      {event.quarter} &bull;{' '}
                      {Math.floor(event.gameClock / 60)}:
                      {(event.gameClock % 60).toString().padStart(2, '0')}
                    </div>
                  </div>
                  <Button
                    size="icon"
