...
interface PlayerStatsModalProps {
  player: PlayerData;
  playerStat: PlayerStatData;
  game: GameInfo;
  homeTeam: TeamInfo;
  awayTeam: TeamInfo;
  onClose: () => void;
  onRecordStat: (statType: string, amount: number) => void;
  onAssignFoul: () => void;
  onSubstitute: () => void;
}

type StatColor = 'blue' | 'purple' | 'indigo' | 'green' | 'cyan' | 'orange';
...
function StatCounter({
  ...
  return (
    <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
      ...
      <Button
        onClick={onIncrement}
        className={`w-full ${COLOR_MAP[color]} text-white rounded-xl py-4 font-bold shadow-md`}
      >
...
export function PlayerStatsModal({
  ...
  onAssignFoul,
  onSubstitute,
}: PlayerStatsModalProps): React.ReactElement {
  const [statUpdates, setStatUpdates] = useState<Record<string, number>>({});

  const incrementStat = (statType: string, amount = 1): void => {
  ...
  const getCurrentValue = (statKey: string): number => {
          ...
          >
            ...
            <div className="text-center mb-6">
              ...
              <div className="text-4xl font-black tracking-tight">
                {homeTeam?.name} {game.homeScore} - {game.awayScore}{' '}
                {awayTeam?.name}
              </div>
            </div>
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="bg-slate-50 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Badge variant="outline" className="text-base">
                      {getCurrentValue('fgm') - getCurrentValue('fgm3')}/
                      {getCurrentValue('fga') - getCurrentValue('fga3')}
                    </Badge>
                  </div>
                  <div className="flex gap-2">
                    <Button
