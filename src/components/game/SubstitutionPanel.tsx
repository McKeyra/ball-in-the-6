...
export function SubstitutionPanel({
  ...
  const [homeOnCourt, setHomeOnCourt] = useState<string[]>(
    game.onCourtHome || []
  );
  const [awayOnCourt, setAwayOnCourt] = useState<string[]>(
    game.onCourtAway || []
  );
  ...
  const updateLineupMutation = useMutation({
    mutationFn: async () => {
      if (homeOnCourt.length !== MAX_ON_COURT) {
        throw new Error(
          `Home team must have exactly ${MAX_ON_COURT} players on court`
        );
      }
      if (awayOnCourt.length !== MAX_ON_COURT) {
        throw new Error(
          `Away team must have exactly ${MAX_ON_COURT} players on court`
        );
      }
    ...
    <Card className="shadow-lg border-0 rounded-2xl mb-6 bg-white">
      <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50">
        <CardTitle className="text-slate-900">
          Substitutions - Select {MAX_ON_COURT} per team
        </CardTitle>
        <Button
          size="icon"
          variant="ghost"
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm font-semibold text-slate-600 bg-slate-100 px-3 py-1 rounded-full">
                {homeOnCourt.length}/{MAX_ON_COURT} selected
              </span>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm font-semibold text-slate-600 bg-slate-100 px-3 py-1 rounded-full">
                {awayOnCourt.length}/{MAX_ON_COURT} selected
              </span>
            </div>
            <div className="grid grid-cols-2 gap-3">
