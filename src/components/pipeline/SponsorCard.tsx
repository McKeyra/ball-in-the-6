...
export function SponsorCard({ sponsor, onClick }: SponsorCardProps) {
  const typeColor =
    SPONSOR_TYPE_COLORS[sponsor?.sponsor_type || ''] || '#c9a962';
  ...
  return (
    <Card
      ...
      onClick={() => sponsor && onClick?.(sponsor)}
    >
      <div className="flex">
        <div className="w-1" style={{ backgroundColor: typeColor }} />
        <div className="flex-1 p-4 space-y-3">
          ...
          <div className="flex items-center gap-2 text-lg font-semibold text-white">
            ...
            {formatCurrency(
              sponsor?.deal_value || sponsor?.sponsorship_value || 0
            )}
          </div>
