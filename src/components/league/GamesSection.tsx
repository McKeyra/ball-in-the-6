...
export function GamesSection(): React.ReactElement {
  const router = useRouter();
  const [activeView, setActiveView] = useState<'live' | 'results' | 'upcoming'>('live');
  const [selectedDivision, setSelectedDivision] = useState('all');

  const { data: games = [] } = useQuery<GameData[]>({
    queryKey: ['games'],
    queryFn: async () => {
      // TODO: Replace with actual API call
      const res = await fetch('/api/games?sort=-game_date&limit=100');
      if (!res.ok) return [];
      return res.json();
    },
  ...
  const filterGamesByDivision = (gamesList: GameData[]): GameData[] => {
    if (selectedDivision === 'all') return gamesList;
    // TODO: Filter by division once team division data is available
    return gamesList;
  };
