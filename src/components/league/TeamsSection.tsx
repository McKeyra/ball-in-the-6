...
const LEAGUES = ['all', 'OSBA', 'Trillium', 'D-League'] as const;
const GENDERS = ['all', 'mens', 'womens', 'boys', 'girls'] as const;

export function TeamsSection(): React.ReactElement {
  ...
  const filteredTeams = teams.filter((team) => {
    ...
    if (filterDivision !== 'all' && team.division !== filterDivision)
      return false;
  ...
  const TeamCard = ({ team }: TeamCardProps): React.ReactElement => (
    <button
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-gray-600">
            ...
            {(team.staff?.length ?? 0) > 0 && (
              <span>&bull; {team.staff!.length} staff</span>
            )}
          </div>
        </div>
      ...
      >
        ...
        <div className="text-center">
          <div className="text-3xl font-bold text-gray-800">
            {teams.filter((t) => t.league === 'OSBA').length}
          </div>
          <div className="text-sm text-gray-500">OSBA</div>
        </div>
        <div className="text-center">
          <div className="text-3xl font-bold text-gray-800">
            {teams.filter((t) => t.league === 'Trillium').length}
          </div>
          <div className="text-sm text-gray-500">Trillium</div>
        </div>
        <div className="text-center">
          <div className="text-3xl font-bold text-gray-800">
            {teams.filter((t) => t.league === 'D-League').length}
          </div>
