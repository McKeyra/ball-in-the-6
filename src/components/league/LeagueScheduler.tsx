...
import { Calendar, Plus, Wand2, Clock, MapPin } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function LeagueScheduler({
  league,
  teams,
  games,
}: LeagueSchedulerProps): React.ReactElement {
  const [showAddGame, setShowAddGame] = useState(false);
  const [newGame, setNewGame] = useState<NewGameData>({
    home_team_id: '',
    away_team_id: '',
    game_date: '',
    location: '',
        ...
        for (let match = 0; match < teamList.length / 2; match++) {
          ...
          if (home && away) {
            schedule.push({
              ...
              status: 'scheduled',
              game_date: new Date(
                Date.now() + (round * 7 + match) * 24 * 60 * 60 * 1000
              ).toISOString(),
  ...
  // Group games by date
  const gamesByDate = games.reduce<Record<string, GameData[]>>((acc, game) => {
                ...
                <div className="space-y-4">
                  {gamesByDate[date].map((game) => {
                    const homeTeam = teams.find(
                      (t) => t.id === game.home_team_id
                    );
                    const awayTeam = teams.find(
                      (t) => t.id === game.away_team_id
                    );
        ...
        <Card>
          <CardContent className="py-20 text-center">
            ...
            <p className="text-slate-600 mb-4">
              Add games manually or auto-generate a schedule
            </p>
          </CardContent>
        </Card>
      )}
