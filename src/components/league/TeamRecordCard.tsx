...
export function TeamRecordCard({
  ...
  record,
  onClick,
}: TeamRecordCardProps): React.ReactElement {
  const gamesPlayed = record.wins + record.losses;
  const winPct =
    gamesPlayed > 0
      ? ((record.wins / gamesPlayed) * 100).toFixed(1)
      : '0.0';
  const ppg =
    gamesPlayed > 0
      ? (record.pointsFor / gamesPlayed).toFixed(1)
      : '0.0';
  const oppg =
    gamesPlayed > 0
      ? (record.pointsAgainst / gamesPlayed).toFixed(1)
      : '0.0';
  const diff =
    gamesPlayed > 0
      ? (
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <div className="text-xs text-gray-500">
            {team.league} &bull; {team.division}
          </div>
        </div>
