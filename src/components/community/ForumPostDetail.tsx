...
export function ForumPostDetail({ post, user, onBack }: ForumPostDetailProps) {
  ...
  const { data: replies = [] } = useQuery<ForumReply[]>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/forum-replies?post_id=${post.id}`);
      return res.json();
    },
  });
                    ...
                    <div className="flex-1">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1 text-sm ${
                          reply.likes?.includes(user?.email)
                            ? 'text-pink-600'
                            : 'text-slate-500'
                        }`}
                      >
