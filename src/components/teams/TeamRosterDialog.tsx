...
export function TeamRosterDialog({
  ...
  open,
  onOpenChange,
}: TeamRosterDialogProps): React.ReactElement {
  ...
  const { data: members = [] } = useQuery<TeamMember[]>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/team-members?team_id=${team.id}`);
      return res.json();
    },
    enabled: !!team.id,
  });
  ...
  const addMutation = useMutation({
    mutationFn: async (data: { team_id: string } & NewMember) => {
      // TODO: Replace with actual API call
        ...
        >
          <TabsList className="bg-white/5 border-white/10 w-full justify-start">
            ...
            <TabsTrigger value="staff">
              Staff & Coaches ({staffMembers.length})
            </TabsTrigger>
          </TabsList>
              ...
              <div className="space-y-4 p-4 bg-white/5 rounded-lg">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(val) =>
                      setNewMember({ ...newMember, player_id: val })
                    }
                  >
