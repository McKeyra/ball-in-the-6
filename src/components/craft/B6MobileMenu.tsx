...
const QUICK_LINKS: QuickLink[] = [
  { name: 'Live Games', icon: PlayCircle, path: '/live-game', color: '#c9a962' },
  { name: 'My Team', icon: Users, path: '/team-dashboard', color: '#4A90E2' },
...
const MENU_SECTIONS: MenuSection[] = [
  {
    title: 'Games & Stats',
    items: [
      { name: 'Court View', path: '/court-view' },
      { name: 'Box Score', path: '/box-score' },
...
interface B6MobileMenuProps {
  isOpen: boolean;
  onClose: () => void;
}

export function B6MobileMenu({ isOpen, onClose }: B6MobileMenuProps) {
              ...
              <div className="bg-[#0f0f0f]/98 backdrop-blur-[30px] rounded-[20px] sm:rounded-[24px] shadow-[0_4px_24px_rgba(0,0,0,0.4),0_12px_48px_rgba(0,0,0,0.5)] overflow-hidden border border-white/[0.08] max-h-[calc(100vh-6rem)] sm:max-h-[calc(100vh-8rem)] overflow-y-auto">
                <div className="sticky top-0 z-10 bg-[#0f0f0f]/98 backdrop-blur-[30px] flex items-center justify-between p-4 sm:p-5 border-b border-white/[0.06]">
                  <div className="flex items-center gap-2 sm:gap-2.5">
                    <Image
                      ...
                      width={32}
                      height={32}
                      className="sm:w-9 sm:h-9 rounded-lg sm:rounded-xl object-contain"
                    />
                      ...
                      <div key={section.title} className="md:mb-0">
                        ...
                        >
                          ...
                          <motion.div
                            animate={{
                              rotate:
                                expandedSection === section.title ? 180 : 0,
                            }}
                            transition={{ duration: 0.2 }}
                          >
                            ...
                            >
                              <div className="pl-3 sm:pl-4 space-y-1 pb-2">
                                {section.items.map((item) => (
                                  ...
                                  >
                                    <span className="text-[14px] sm:text-[15px] text-white/70">
                                      {item.name}
                                    </span>
                ...
                <div className="sticky bottom-0 bg-[#0f0f0f]/98 backdrop-blur-[30px] p-3 sm:p-4 border-t border-white/[0.06]">
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <button
                      onClick={() => handleNavigate('/player-dashboard')}
                  ...
                  <button className="w-full bg-[#c9a962] text-[#0f0f0f] py-3.5 sm:py-4 rounded-full font-semibold text-[15px] sm:text-[16px] hover:bg-[#d4b46d] transition-all duration-200 active:scale-[0.98] min-h-[48px]">
                    Sign Out
                  </button>
                </div>
              </div>
