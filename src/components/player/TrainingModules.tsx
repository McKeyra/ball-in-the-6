...
export function TrainingModules(_props: TrainingModulesProps) {
  return (
            ...
            <CardContent className="p-5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <h3 className="font-bold text-white mb-2">{mod.title}</h3>
                  <div className="flex items-center gap-2 text-xs">
                    <span
                      className={`px-2 py-1 rounded ${DIFFICULTY_COLORS[mod.difficulty]}`}
                    >
                      {mod.difficulty}
                    </span>
                ...
                {mod.locked && <Lock className="w-6 h-6 text-gray-400" />}
              </div>
