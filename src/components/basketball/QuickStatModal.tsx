...
export function QuickStatModal({
  ...
  players,
  onClose,
}: QuickStatModalProps): React.ReactElement {
  ...
  const createEventMutation = useMutation({
    mutationFn: async ({
      ...
      const gameUpdates: Record<string, number> = {};
      if (stat.points > 0) {
        const scoreField =
          player.team === 'home' ? 'home_score' : 'away_score';
        gameUpdates[scoreField] =
          (game[scoreField as keyof GameData] as number) + stat.points;
      }
      ...
      if (Object.keys(gameUpdates).length > 0) {
        await fetch(`/api/games/${game.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(gameUpdates),
        });
      }
    },
    onSuccess: () => {
