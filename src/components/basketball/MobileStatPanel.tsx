...
type StatType = 'made' | 'missed' | string;

interface PendingMakeMiss {
  made?: number;
  missed?: number;
}

interface PendingStats {
  [key: string]: number | PendingMakeMiss | unknown[];
}

interface MobileStatPanelProps {
  teamColor: string;
  onStatChange: (stat: string, type: StatType, value: number) => void;
  pendingStats: PendingStats;
}

interface StatSwipeButtonProps {
  label: string;
  stat: string;
  type?: string;
  variant?: 'make' | 'miss';
...
export function MobileStatPanel({
  teamColor,
  onStatChange,
  pendingStats,
}: MobileStatPanelProps): React.ReactElement {
  const [isExpanded, setIsExpanded] = useState(false);
  const [activeCategory, setActiveCategory] = useState<'scoring' | 'other'>(
    'scoring'
  );
  ...
  const StatSwipeButton = ({
    ...
    icon,
    color,
  }: StatSwipeButtonProps): React.ReactElement => {
    const pending = pendingStats[stat];
    const isPending =
      type === 'made'
        ? (pending as PendingMakeMiss)?.made
          ? (pending as PendingMakeMiss).made! > 0
          : false
        : type === 'missed'
          ? (pending as PendingMakeMiss)?.missed
            ? (pending as PendingMakeMiss).missed! > 0
            : false
      ...
      <motion.div
        ...
        onTouchEnd={(e) =>
          handleTouchEnd(
            ...
            stat,
            type === 'make' ? 'made' : type || stat
          )
        }
        onClick={() =>
          onStatChange(stat, type === 'make' ? 'made' : type || stat, 1)
        }
      >
        <motion.div
        ...
        >
          ...
          {isPending && (
            <motion.div
              ...
              style={{
                background:
                  variant === 'miss' ? '#6B7280' : color || teamColor,
              }}
            >
              {pendingValue}
            </motion.div>
  ...
  const hasPending = Object.values(pendingStats).some(
    (v) =>
      (typeof v === 'number' && v > 0) ||
      ((v as PendingMakeMiss)?.made !== undefined &&
        ((v as PendingMakeMiss).made! > 0 ||
          (v as PendingMakeMiss).missed! > 0)) ||
        ...
        {isExpanded && (
          ...
          >
            <div className="p-4 max-h-96 overflow-auto">
              ...
              >
                {(['scoring', 'other'] as const).map((cat) => (
                  <motion.button
                    key={cat}
                    ...
                    style={{
                      background:
                        activeCategory === cat ? teamColor : '#e0e0e0',
