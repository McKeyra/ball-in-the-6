...
type FreeThrowResult = 'made' | 'missed';

const MAX_FREE_THROWS = 3;

interface FreeThrowTrackerProps {
  freeThrows: FreeThrowResult[];
  onChange: (freeThrows: FreeThrowResult[]) => void;
  color: string;
}

export function FreeThrowTracker({
  freeThrows,
  onChange,
  color,
}: FreeThrowTrackerProps): React.ReactElement {
  const addFreeThrow = (result: FreeThrowResult): void => {
    if (freeThrows.length < MAX_FREE_THROWS) {
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          disabled={freeThrows.length >= MAX_FREE_THROWS}
          className="py-3 rounded-xl text-sm font-semibold disabled:opacity-40"
          style={{
            background: color,
            color: 'white',
            boxShadow:
              '4px 4px 8px rgba(0,0,0,0.1), -2px -2px 4px rgba(255,255,255,0.3)',
            border: 'none',
          }}
        >
          Made
