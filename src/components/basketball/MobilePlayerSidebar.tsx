...
interface MobilePlayerSidebarProps {
  players: PlayerData[];
...
export function MobilePlayerSidebar({
  players,
  currentPlayerId,
  gameId,
  teamColor,
  isOpen,
  onClose,
}: MobilePlayerSidebarProps): React.ReactElement {
  const router = useRouter();

  const switchToPlayer = (playerId: string): void => {
    router.push(`/player-profile?playerId=${playerId}&gameId=${gameId}`);
    onClose();
  };
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col items-end text-xs text-gray-600">
                      ...
                      <div>
                        {player.rebounds_off + player.rebounds_def}r
                      </div>
