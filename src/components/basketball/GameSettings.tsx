...
export function GameSettings({
  ...
  const exportToPDF = async (): Promise<void> => {
    try {
      // TODO: Replace with actual PDF export API call
      const response = await fetch(`/api/games/${game.id}/export-pdf`, {
        method: 'POST',
      });
        ...
        <div className="flex-1 overflow-auto p-6 space-y-6">
              ...
              {editGameInfo ? (
                <>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e: ChangeEvent<HTMLInputElement>) =>
                        setGameInfo({ ...gameInfo, location: e.target.value })
                      }
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button
                      onClick={() => {
                        updateGameMutation.mutate(gameInfo as unknown as Record<string, unknown>);
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-semibold">
                        {game.location || 'Not set'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-xs text-gray-500">Date:</span>
                      <span className="text-sm font-semibold">
                        {game.game_date || 'Not set'}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-xs text-gray-500">Time:</span>
                      <span className="text-sm font-semibold">
                        {game.game_time || 'Not set'}
                      </span>
                    </div>
                    <div className="flex justify-between">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button
                onClick={() =>
                  updateGameMutation.mutate({
                    quarter: Math.min(game.quarter + 1, MAX_QUARTER),
          ...
          >
            <div className="text-xs text-gray-500 text-center">
              Tip: Use Event Feed to delete individual events if errors occur.
              Manual score adjustments don&apos;t affect event history.
            </div>
          </div>
        </div>
      </div>
