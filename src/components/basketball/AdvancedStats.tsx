...
import { TrendingUp, Target, Percent, Activity, type LucideIcon } from 'lucide-react';
...
export function AdvancedStats({ player }: AdvancedStatsProps): React.ReactElement {
  const efg =
    player.fga > 0
      ? (((player.fgm + 0.5 * player.three_pm) / player.fga) * 100).toFixed(1)
      : '0.0';

  const ts =
    player.fga > 0
      ? (
          (player.points / (2 * (player.fga + 0.44 * player.fta))) *
          100
        ).toFixed(1)
      : '0.0';
