...
function PlayerSchedule(): React.ReactElement {
  return (
    <div className="p-6 space-y-4 animate-in fade-in slide-in-from-bottom-4">
      ...
      {[1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="neu-flat p-4 rounded-xl flex items-center justify-between"
        >
          ...
          <Button
            size="sm"
            className="rounded-full bg-blue-600/20 text-blue-400 hover:bg-blue-600 hover:text-white"
          >
            Tickets
          </Button>
        </div>
...
function PlayerConnections(): React.ReactElement {
  return (
    <div className="p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div className="neu-flat p-5 rounded-2xl">
        <h4 className="text-white font-bold mb-4 text-sm uppercase tracking-wider text-gray-500">
          Current Team
        </h4>
      ...
      <div className="neu-flat p-5 rounded-2xl">
        ...
        <div className="space-y-4">
          {['Head Coach', 'Assistant Coach'].map((role, i) => (
            <div
              key={i}
              className="flex items-center gap-3 p-2 hover:bg-[#252525] rounded-lg transition-colors cursor-pointer"
            >
...
function FanConnections(): React.ReactElement {
  const familyMembers = [
  ...
  const programs: FanProgram[] = [
    { name: 'Summer Youth League 2024', duration: 'Jun 1 - Aug 31', owing: 150.0 },
    { name: 'Winter Skills Camp', duration: 'Dec 15 - Jan 15', owing: 0.0 },
  ];
        ...
        <div className="neu-flat p-5 rounded-2xl">
          ...
          <div className="space-y-3">
            {['Coach Mike', 'Coach Sarah'].map((name, i) => (
              <div
                key={i}
                className="flex items-center gap-3 p-2 hover:bg-[#252525] rounded-lg transition-colors"
              >
      ...
      <div className="neu-flat p-5 rounded-2xl">
        ...
        <div className="space-y-3">
          {programs.map((program, i) => (
            <div
              key={i}
              className="flex items-center justify-between p-4 rounded-xl bg-[#151515] border border-white/5"
            >
              <div>
              ...
              <div className="text-right">
                ...
                <div
                  className={`text-xl font-mono font-bold ${program.owing > 0 ? 'text-red-500' : 'text-green-500'}`}
                >
...
export function ChatWindow({
  ...
  const { data: messages, isLoading } = useQuery<Message[]>({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `/api/messages?user_id=${user.id}&current_user=${currentUser?.email}`
      );
      return res.json();
    },
  ...
  }, [messages, user?.id, currentUser?.role]);

  const handleTranslate = async (msgId: string, text: string): Promise<void> => {
    try {
      ...
      if (data?.translated_text) {
        setTranslations((prev) => ({ ...prev, [msgId]: data.translated_text }));
      }
    } catch {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (newMsg: Partial<Message>) => {
      // TODO: Replace with actual API call
        ...
        <div className="p-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              />
              {user.online && (
                <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-[#202020] rounded-full" />
              )}
            </div>
            <div>
              <h3 className="font-bold text-white">{user.name}</h3>
              <span className="text-xs text-blue-400 capitalize">
                {category?.slice(0, -1) || 'User'}
              </span>
            </div>
          </div>
        ...
        {activeTab === 'chat' && (
          <>
            ...
            <div className="px-4 pt-2 bg-[#202020]">
              ...
              {aiCategory && (
                <div className="inline-block px-2 py-0.5 rounded-md bg-purple-500/20 text-purple-300 text-[10px] font-bold uppercase mb-2 border border-purple-500/30">
                  {aiCategory}
                </div>
              )}
