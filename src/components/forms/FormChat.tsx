...
import { Send, CheckCircle2, Sparkles, LayoutList } from 'lucide-react';
...
const getFieldQuestion = (field: FormField): string => {
...
const getAcknowledgment = (fieldName: string, value: string): string => {
...
function QuickReplyButton({
  ...
  onClick,
  disabled,
}: QuickReplyButtonProps): React.ReactElement {
  return (
    <button
      type="button"
...
function ChatProgress({ current, total }: ChatProgressProps): React.ReactElement {
...
export function FormChat({
  ...
  onComplete,
  onSwitchMode,
}: FormChatProps): React.ReactElement {
  ...
  const simulateTyping = useCallback(async (duration = 800): Promise<void> => {
    setIsTyping(true);
    await new Promise((resolve) => setTimeout(resolve, duration));
    setIsTyping(false);
  }, []);

  const getQuickRepliesForField = useCallback(
  ...
  const askNextQuestion = useCallback(async (): Promise<void> => {
  ...
  const handleUserResponse = useCallback(
    async (response: string): Promise<void> => {
      ...
      addMessage('user', response, currentField?.name ?? null);
      setInputText('');
      setQuickReplies([]);
  ...
  const handleQuickReply = (option: string): void => {
  ...
  useEffect(() => {
    if (
      currentFieldIndex > 0 &&
      currentFieldIndex <= allFields.length &&
      messages.length > 0
    ) {
      const timer = setTimeout(() => {
        askNextQuestion();
