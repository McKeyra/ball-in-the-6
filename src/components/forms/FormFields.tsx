...
`;
const HAS_VALUE_STYLES = 'border-white/[0.12] bg-white/[0.08]';
const ERROR_STYLES =
...
export function PillsField({
  ...
  maxSelect,
  className = '',
}: PillsFieldProps): React.ReactElement {
    ...
    >
      ...
      {maxSelect && (
        <p className="text-white/40 text-xs mt-2">
          ...
          {value.length > 0 && `(${value.length}/${maxSelect})`}
        </p>
      )}
    </FieldWrapper>
  );
...
export function CardsField({
  ...
  multiple = false,
  className = '',
}: CardsFieldProps): React.ReactElement {
        ...
        {options.map((option) => {
          ...
          return (
            ...
            >
              ...
              {IconComponent && (
                <div
                  className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${selected ? 'bg-[#c9a962]/20' : 'bg-white/[0.06]'}`}
                >
                  <IconComponent
                    className={`w-5 h-5 ${selected ? 'text-[#c9a962]' : 'text-white/60'}`}
                  />
                </div>
              )}
              <p
                className={`font-medium ${selected ? 'text-white' : 'text-white/80'}`}
              >
                {option.label}
              </p>
              {option.description && (
      ...
      >
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-white/60 text-sm">
            <span className="text-[#c9a962]">Click to upload</span> or drag and
            drop
          </p>
          {accept && (
