...
interface MultiDivisionSchedulerProps {
  teams: unknown[];
...
export function MultiDivisionScheduler({
  scheduleItems,
}: MultiDivisionSchedulerProps) {
  ...
  const conflictCount = checkConflicts();
      ...
      <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">
                Venue or time overlaps need resolution
              </p>
            </div>
            <Button
              size="sm"
        ...
        <div className="grid gap-3">
          {scheduleItems.slice(0, 10).map((item) => (
            <div
              key={item.id}
              className="p-4 bg-white/5 rounded-lg border border-white/10 hover:border-[#D0FF00]/50 transition-all"
            >
