...
export function RefAssignor({ scheduleItems }: RefAssignorProps) {
  const upcomingGames = scheduleItems
    .filter(
      (item) => item.type === 'game' && new Date(item.date) >= new Date()
    )
    .slice(0, 8);
                  ...
                  <div className="flex items-center gap-2">
                    <Select>
                      ...
                      <SelectContent className="bg-[#1A1A1A] border-white/10">
                        {MOCK_REFEREES.filter((r) => r.available).map(
                          (ref) => (
                            <SelectItem
                              key={ref.id}
                    ...
                    <Button
                      size="sm"
                      className="bg-[#D0FF00] text-[#0A0A0A]"
                    >
            ...
            {MOCK_REFEREES.map((ref) => (
              <div key={ref.id} className="p-3 bg-white/5 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={`w-2 h-2 rounded-full ${ref.available ? 'bg-green-400' : 'bg-red-400'}`}
                  />
                </div>
