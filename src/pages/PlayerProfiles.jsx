    ...
    <div className="min-h-screen bg-[#0f0f0f] p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white/90">Player Profiles</h1>
            <p className="text-white/60">{players.length} registered players</p>
          </div>
          <Button
        ...
        {filteredPlayers.length === 0 ? (
          ...
          >
            ...
            <h3 className="text-xl font-semibold text-white/70 mb-2">No Players Found</h3>
            ...
            {filteredPlayers.map((player) => {
              ...
              return (
                <button
                  ...
                  className="text-left rounded-3xl p-5 hover:scale-[1.02] transition-transform"
                  style={{
                    background: '#0f0f0f',
                    boxShadow: '8px 8px 16px rgba(0,0,0,0.1), -8px -8px 16px rgba(255,255,255,0.7)'
                  }}
                >
                    ...
                    <div className="flex-1">
                      ...
                      {team && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          />
                          <span className="text-xs text-white/60">{team.team_name}</span>
                        </div>
                      )}
                    </div>
                    {player.status && player.status !== 'active' && (
                      <span className={`px-2 py-1 rounded-lg text-xs font-semibold ${
                        player.status === 'injured' ? 'bg-red-100 text-red-700' : 'bg-white/[0.06] text-white/60'
                      }`}>
                        {player.status}
                      </span>
                    )}
                  ...
                  >
                    <div className="flex items-center gap-1 mb-2 text-xs text-white/40">
                      <Trophy className="w-3 h-3" />
                      <span>Career Stats ({player.career_games || 0} games)</span>
                    </div>
                    <div className="grid grid-cols-4 gap-2 text-center">
                      <div>
                        <div className="text-lg font-bold text-white/90">{getPlayerAvg(player, 'career_points')}</div>
                        <div className="text-xs text-white/40">PPG</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-white/90">{getPlayerAvg(player, 'career_rebounds')}</div>
                        <div className="text-xs text-white/40">RPG</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-white/90">{getPlayerAvg(player, 'career_assists')}</div>
                        <div className="text-xs text-white/40">APG</div>
                      </div>
                      <div>
                        <div className="text-lg font-bold text-white/90">{getPlayerAvg(player, 'career_steals')}</div>
                        <div className="text-xs text-white/40">SPG</div>
                      </div>
                    </div>
                  </div>
                  ...
                  {(player.career_fga > 0 || player.career_three_pa > 0 || player.career_fta > 0) && (
                    <div className="flex justify-around mt-3 text-xs text-white/40">
                      {player.career_fga > 0 && (
