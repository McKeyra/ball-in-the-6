    ...
    <div className="min-h-screen bg-[#0f0f0f] p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-white/40 mt-1">Celebrating excellence in our community</p>
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          {/* Player of the Week */}
          <Card className="bg-gradient-to-br from-yellow-500/20 to-orange-500/20 border-yellow-500/30 overflow-hidden">
            <CardContent className="p-6">
              <div className="flex items-center gap-2 mb-4">
                <Star className="w-6 h-6 text-yellow-500" />
                <h3 className="text-lg font-bold text-white">Player of the Week</h3>
              </div>
              {playerOfWeek ? (
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xl font-bold text-white">{playerOfWeek.recipient_name}</p>
                    <p className="text-yellow-300">{playerOfWeek.team_name}</p>
                ...
                <div className="text-center py-8">
                  <Sparkles className="w-12 h-12 mx-auto mb-2 text-yellow-500/50" />
                  <p className="text-white/40">No player of the week yet</p>
                  <p className="text-sm text-white/30">Nominate a standout player!</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 border-purple-500/30 overflow-hidden">
            <CardContent className="p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-bold text-white">Team of the Month</h3>
              </div>
              {teamOfMonth ? (
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white">
                    <Users className="w-10 h-10" />
                  </div>
                  <div>
                    <p className="text-xl font-bold text-white">{teamOfMonth.recipient_name}</p>
                    <p className="text-purple-300">{new Date(teamOfMonth.month_of + '-01').toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>
                ...
                <div className="text-center py-8">
                  <Sparkles className="w-12 h-12 mx-auto mb-2 text-purple-500/50" />
                  <p className="text-white/40">No team of the month yet</p>
                  <p className="text-sm text-white/30">Nominate a dominant team!</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <Tabs value={selectedTab} onValueChange={setSelectedTab} className="mb-6">
          <TabsList className="bg-white/[0.08] border-white/[0.08] flex-wrap h-auto p-1">
            <TabsTrigger value="all" className="data-[state=active]:bg-orange-600">All Awards</TabsTrigger>
            {awardTypes.map(type => (
              <TabsTrigger key={type.id} value={type.id} className="data-[state=active]:bg-orange-600">
                {type.label}
              </TabsTrigger>
            ))}
          </TabsList>
                  ...
                  <Card className="bg-white/[0.05] border-white/[0.06] overflow-hidden hover:border-white/[0.08] transition-all">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${awardType?.color || 'from-white/20 to-white/30'} flex items-center justify-center`}>
                          <Icon className="w-6 h-6 text-white" />
                        </div>
                        <Badge variant="outline" className="text-white/40 border-white/[0.08]">
                          {awardType?.label}
                        </Badge>
                      </div>
                      
                      <h3 className="text-lg font-bold text-white mb-1">{award.recipient_name}</h3>
                      {award.team_name && (
                        <p className="text-sm text-white/40 mb-2">{award.team_name}</p>
                      )}
                      {award.reason && (
                        <p className="text-sm text-white/30 mb-4 line-clamp-2">{award.reason}</p>
                      )}
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1 px-3 py-1 rounded-full text-sm transition-all ${
                            hasVoted 
                              ...
                              : 'bg-white/[0.08] text-white/40 hover:bg-white/[0.08]'
                          }`}
                        >
        ...
        {filteredAwards.length === 0 && (
          <Card className="bg-white/[0.05] border-white/[0.06]">
            <CardContent className="py-16 text-center">
              <Trophy className="w-16 h-16 mx-auto mb-4 text-white/[0.08]" />
              <h3 className="text-xl font-semibold text-white mb-2">No awards yet</h3>
              <p className="text-white/40 mb-4">Be the first to recognize outstanding performance!</p>
              <Button onClick={() => setShowNominate(true)} className="bg-gradient-to-r from-yellow-500 to-orange-500">
                Submit Nomination
              </Button>
            </CardContent>
          </Card>
