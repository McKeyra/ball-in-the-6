...
export function TeamListPage(): React.ReactElement {
    ...
    <div className="min-h-screen bg-[#0f0f0f] p-4 md:p-6 lg:p-8 pb-24">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 md:mb-8">
          ...
          <Link
            href="/teams/create"
            className="min-h-[44px] px-4 md:px-6 font-semibold flex items-center rounded-md"
            style={{ backgroundColor: '#1a1a1a', color: 'white', boxShadow: '4px 4px 12px rgba(0,0,0,0.2)' }}
          >
        ...
        {teams.length === 0 ? (
          <div
            ...
            style={{ background: '#0f0f0f', boxShadow: 'inset 4px 4px 8px rgba(0,0,0,0.1), inset -4px -4px 8px rgba(255,255,255,0.7)' }}
          >
            ...
            <Link
              href="/teams/create"
              className="min-h-[44px] px-6 inline-flex items-center rounded-md"
              style={{ backgroundColor: '#1a1a1a', color: 'white', boxShadow: '4px 4px 12px rgba(0,0,0,0.2)' }}
            >
              <Plus className="w-5 h-5 mr-2" />
              Create Team
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {teams.map((team) => (
              <Link
                ...
                style={{ background: '#0f0f0f', boxShadow: '6px 6px 12px rgba(0,0,0,0.1), -6px -6px 12px rgba(255,255,255,0.7)', borderRadius: '24px', padding: '20px', display: 'block' }}
              >
                <div className="flex items-center gap-3 md:gap-4 mb-4">
                  {team.logo_url ? (
                    <img src={team.logo_url} alt={team.team_name} className="w-14 h-14 md:w-16 md:h-16 rounded-full object-cover" />
                  ) : (
                    <div
